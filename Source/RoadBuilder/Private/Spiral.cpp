...
static double xFresnel_Auxiliary_Cosine_Integral(double x);
static double xFresnel_Auxiliary_Sine_Integral(double x);
static double Power_Series_C(double x);
static double xChebyshev_Tn_Series(double x, const double a[], int degree);
...
double lfresnel_sin_integral(double x)
{
    double sqrt_2_o_pi = 7.978845608028653558798921198687637369517e-1L;
    return fresnel_sin(x / sqrt_2_o_pi);
}

double lfresnel_cos_integral(double x)
{
    double sqrt_2_o_pi = 7.978845608028653558798921198687637369517e-1L;
    return fresnel_cos(x / sqrt_2_o_pi);
}

double fresnel_sin_integral(double x)
{
    return static_cast<double>(lfresnel_sin_integral(static_cast<double>(x)));
}

double fresnel_cos_integral(double x)
{
    return static_cast<double>(lfresnel_cos_integral(static_cast<double>(x)));
}
...
static double const sqrt_2pi = 2.506628274631000502415765284811045253006L;
...
double Fresnel_Auxiliary_Sine_Integral(double x)
{
    return (double)xFresnel_Auxiliary_Sine_Integral((double)x);
}
...
static double sin_Chebyshev_Expansion_0_1(double x);
static double sin_Chebyshev_Expansion_1_3(double x);
static double sin_Chebyshev_Expansion_3_5(double x);
static double sin_Chebyshev_Expansion_5_7(double x);
static double sin_Asymptotic_Series(double x);
...
static double sin_Chebyshev_Expansion_0_1(double x)
{
    ...
    static const double midpoint = 0.5L;
    static const double scale = 0.5L;
...
static double sin_Chebyshev_Expansion_1_3(double x)
{
    ...
    static const int degree = sizeof(c) / sizeof(double) - 1;
    static const double midpoint = 2.0L;
...
static double sin_Chebyshev_Expansion_3_5(double x)
{
    ...
    static const int degree = sizeof(c) / sizeof(double) - 1;
    static const double midpoint = 4.0L;
    return xChebyshev_Tn_Series((x - midpoint), c, degree);
}
...
static double sin_Chebyshev_Expansion_5_7(double x)
{
    ...
    static const int degree = sizeof(c) / sizeof(double) - 1;
    static const double midpoint = 6.0L;
    return xChebyshev_Tn_Series((x - midpoint), c, degree);
}
...
double Fresnel_Auxiliary_Cosine_Integral(double x)
{
    return (double)xFresnel_Auxiliary_Cosine_Integral((double)x);
}

static double cos_Chebyshev_Expansion_0_1(double x);
static double cos_Chebyshev_Expansion_1_3(double x);
static double cos_Chebyshev_Expansion_3_5(double x);
static double cos_Chebyshev_Expansion_5_7(double x);
static double cos_Asymptotic_Series(double x);
...
double xFresnel_Cosine_Integral(double x);
...
double Fresnel_Cosine_Integral(double x)
{
    return (double)xFresnel_Cosine_Integral((double)x);
}
...
FVector2D GetSpiralPos(double initX, double initY, double initTheta, double initCurv, double dCurv, double length)
{
    FVector2D pos;
    ...
    {
        double a = 1.0 / sqrt(DOUBLE_PI * abs(dCurv));
        ...
        if (dCurv < 0.0) x *= -1.0;
        pos.X = DOUBLE_PI * a * (x * cos(theta) - y * sin(theta));
        pos.Y = DOUBLE_PI * a * (x * sin(theta) + y * cos(theta));
    }
    pos.X += initX;
    pos.Y += initY;
    return pos;
...
double GetSpiralRadian(double initTheta, double initCurv, double dCurv, double length)
{
    double R = dCurv * length * length / 2.0 + initCurv * length + initTheta;
    //  Only Diff need clamp to -PI ~ PI?
    //  if (R <= -DOUBLE_PI) R += 2 * DOUBLE_PI;
    //  if (R > DOUBLE_PI) R -= 2 * DOUBLE_PI;
    return R;
}

double ComputeSpiralLength(double theta0, double theta1, double k0, double k1)
{
    return (theta1 - theta0) / ((k1 - k0) / 2 + k0);
}
